classdef Classifier_HierarchicalBCorr_M
    ...
    methods
           function c= Classifier_HierarchicalBCorr_M(view,SRG)
               ...
               if(level>0)% using SRG and based on level traveles SRG graph and extract correlation based on semi RCE
                   ...
                   if backindexkeepflag==1
                       for j=2:size(view,2)  %j=1 =target table and do not need back index
                        ...
                        Set=Setoflabel';
                               ...
                               [accuracy,classifiers,prevotingUnlabel]=SemiRCE_Ensemble( view{1,backindexarr(1,i)}{1,2},view{1,backindexarr(1,i)}{1,3},view{1,backindexarr(1,i)}{1,4},view{1,j}{1,2},view{1,j}{1,3},view{1,backindexarr(1,i)}{1,5},view{1,backindexarr(1,i)}{1,6},view{1,j}{1,6},view{1,backindexarr(1,i)}{1,7},Setoflabel,dim,ensembelsize,0);
                          ...
                          if m>0
                            indexback=backindexarr(1,indexmax(1,1))                         
                   ...
                   for indexinlevel=1: size(indexoflevel,2)
                     ...
                     %   code=code+1;
                       ...
                       [accuracyoftraindata,classifiers,prevotingUnlabel,predicationTest]=SemiRCE_Ensemble( view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,2},view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,3},view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,4},view{1,indexoflevel(1,indexinlevel)}{1,2},view{1,indexoflevel(1,indexinlevel)}{1,3},view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,5},view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,6},view{1,indexoflevel(1,indexinlevel)}{1,6},view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,7},Setoflabel,dim,ensembelsize,flagsim);

                        WofClassifier{1,code}=accuracyoftraindata;
  %{
                      data1{1,1}=view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,2};%train data view1
                      data1{1,2}=view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,3};%unlabel data view2
                      data1{1,3}=view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,6};
                      ...
                      labelforrce{1,1}=view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,4};
                      labelforrce{1,2}=view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,5};
                      labelforrce{1,3}=view{1,backindexKeep(indexoflevel(1,indexinlevel),1)}{1,7};
                      ...
                      [acc ,prevotingUnlabel,predicationTest]= MultiSemi_RCE( data1,labelforrce,Setoflabel,dim,ensembelsize,flagsim);
                       ...
                       if(level==1)% level =3 in loan database that final
                           flag=0;
                       end
                   end
